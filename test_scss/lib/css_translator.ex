defmodule TestSCSS.CSSTranslator do
  @moduledoc false
  alias TestSCSS.CSSParser
  ...
  defmacro scope_id do
    ...
    TestSCSS.CSSTranslator.scope_id(mod, func)
  end

  defmacro scope_attr do
    mod = __CALLER__.module
    {func, _} = __CALLER__.function
    TestSCSS.CSSTranslator.scope_attr(mod, func)
  end

  def translate!(css, opts \\ [])

  def translate!(css, opts) when is_binary(css) do
  ...
  def translate!(ast, opts) do
    ...
    {updated_tokens, state} = translate(ast, [], state)
